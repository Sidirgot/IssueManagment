    ...
    <modal name="edit-developers" height="auto" :adaptive="true" :clickToClose="false" @before-open="fetchDevelopers">
        <div class="p-6">
            <div class="flex flex-col md:flex-row justify-between items-center border-b border-gray-200 pb-2">
                <h1 class="self-start text-xl text-gray-600">Update Project Developers</h1>

                <button @click="$modal.hide('edit-developers')" class="self-end btn btn-teal text-white mx-2">X</button>
            </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap w-full p-3 bg-gray-300 rounded my-2">
                        <span v-for="developer in project.project_developers" :key="developer.id" class="bg-teal-600 text-white rounded px-1 mx-2 my-1">
                            {{ developer.name }}
                            <button @click.prevent="removeDeveloper(developer)" class="bg-teal-700 text-sm hover:opacity-50 px-2 my-1 rounded" >x</button>
                        </span>
                    </div>
